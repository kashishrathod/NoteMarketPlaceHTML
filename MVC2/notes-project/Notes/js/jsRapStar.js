(function ($) {
  $.fn.jsRapStar = function (options) {
    return this.each(function () {
      ...
      let starH = Array(this.opt.length + 1).join(
        "<span>" + this.opt.star + "</span>"
      );
      $(this)
        .empty()
        .addClass("rapStar")
      ...
      this.StarF = $("<div>")
        .addClass("rapStarFront")
        .css({ color: this.opt.colorFront })
        .html(starH)
        .width(widthValue)
        .appendTo(this);
      if (this.opt.enabled) {
        $(this).bind({
          mousemove: function (e) {
            ...
            if (this.opt.step)
              widthCurrent =
                Math.floor(widthCurrent / widthSingle + 1) * widthSingle;
            this.StarF.width(widthCurrent);
            if (this.opt.onMousemove) {
              let x = (widthCurrent / widthSingle).toFixed(2);
              this.opt.onMousemove.call(this, x);
            }
          },
          ...
          click: function (e) {
            ...
            if (this.opt.onClick) this.opt.onClick.call(this, this.opt.value);
          },
        });
      } else $(this).addClass("rapStarDisable");
    });
  };
})(jQuery);
